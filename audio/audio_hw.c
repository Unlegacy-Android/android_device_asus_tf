...
static ssize_t read_frames(struct tiny_stream_in *in, void *buffer, ssize_t frames)
{
	...
	while (frames_wr < frames) {
		...
		if (in->resampler != NULL) {
			...
			if (buf.raw != NULL) {
				memcpy((char *)buffer +
				frames_wr * audio_stream_in_frame_size(&in->stream), buf.raw,
				  ...
				  audio_source_t source __unused)
{
	...
	if (in->requested_rate != in->config.rate) {
		...
		ret = create_resampler(in->config.rate, in->requested_rate,
		...
		for (i = 0; i < sizeof(dev_names) / sizeof(dev_names[0]); i++) {
			if (strcmp(dev_names[i].name, name) == 0) {
				...
				dev_cfg = realloc(s->adev->dev_cfgs,
					(s->adev->num_dev_cfgs + 1) * sizeof(*dev_cfg));
				if (!dev_cfg) {
...
static int adev_config_parse(struct tiny_audio_device *adev)
{
	...
	switch(atoi(project_id)) {
		...
		case 0x04:  // TF700T
		case 0x05:  // TF300TL
			strlcpy(file, "/system/etc/tiny_hw_rt5631.xml", sizeof(file));
			break;

		case 0x02:  // TF300T
			strlcpy(file, "/system/etc/tiny_hw_wm8903.xml", sizeof(file));
			break;

...
#define BRIGHTNESS_FILE "/sys/class/backlight/pwm-backlight/brightness"
#define PERSISTENCE_FILE "/sys/class/backlight/pwm-backlight/low_persistence"
...
static int write_int(char const *path, int value)
{
	...
	if (fd >= 0) {
		...
		size_t amt = write(fd, buffer, bytes);
		close(fd);
		       ...
		       struct hw_device_t **device)
{
	...
	else if ((0 == strcmp(LIGHT_ID_KEYBOARD, name)) || (0 == strcmp(LIGHT_ID_BUTTONS, name)) ||
		(0 == strcmp(LIGHT_ID_BATTERY, name)) || (0 == strcmp(LIGHT_ID_NOTIFICATIONS, name)) ||
		(0 == strcmp(LIGHT_ID_ATTENTION, name)) || (0 == strcmp(LIGHT_ID_BLUETOOTH, name)) ||
		(0 == strcmp(LIGHT_ID_WIFI, name)))
		set_light = NULL;
	else
		return -EINVAL;
...
struct hw_module_t HAL_MODULE_INFO_SYM =
{
	...
	.name = "NVIDIA Cardhu lights module",
	.author = "NVIDIA",

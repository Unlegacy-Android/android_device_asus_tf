...
{
    int index = handleToDriver(handle);
    ALOGE("%s: index=%d", __func__, index);
    if (index < 0) return index;
    ALOGE("%s: index=%d, handle=%d and period_ns=%lld, timeout=%lld", __func__, index, handle, period_ns, timeout);
                         ...
                         int flags, int64_t period_ns, int64_t timeout)
{
    FUNC_LOG;
    sensors_poll_context_t* ctx = (sensors_poll_context_t*) dev;
    ALOGE("%s: handle=%d and period_ns=%lld, timeout=%lld", __func__, handle, period_ns, timeout);
